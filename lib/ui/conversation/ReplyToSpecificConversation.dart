...
class ReplyToSpecificConversation extends StatefulWidget {
  PostsOfThreads conversation;
  String receivedMessage="";
  ReplyToSpecificConversation(this.conversation, this.receivedMessage, {Key? key}) : super(key: key);

  @override
  State<ReplyToSpecificConversation> createState() => _ReplyToSpecificConversationState();
}

class _ReplyToSpecificConversationState extends State<ReplyToSpecificConversation> {
                              ...
                              onTap: (){
                                if(_firstMessageControllar.text.isNotEmpty||_lastMessageControllar.text.isNotEmpty){
                                  ...
                                  if(message.contains("[/QUOTE]")){
                                    ...
                                    var realMessage="[QUOTE=\"${widget.conversation.User?.username}, post: ${widget.conversation.post_id}, member: ${widget.conversation.User?.user_id}\"]${exactString}[/QUOTE]";
                                    postReply(context, _firstMessageControllar.text+realMessage+_lastMessageControllar.text);
                                  }else{
                                    var realMessage="[QUOTE=\"${widget.conversation.User?.username}, post: ${widget.conversation.post_id}, member: ${widget.conversation.User?.user_id}\"]${message}[/QUOTE]";
                                    postReply(context, _firstMessageControllar.text+realMessage+_lastMessageControllar.text);
                                  }
                                }
                              },
...
showLoaderDialog(context, "Sending");
    ...
    var response = await ApiClient(
        ...
        MyDataClass.api_key, MyDataClass.myUserId, widget.conversation.conversation_id,message,attachmentKey);
    Navigator.pop(context);
    Navigator.pop(context);
